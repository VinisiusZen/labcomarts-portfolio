...
const bgStars = Array.from({ length: 170 }, (_, i) => ({
  x: (i * 197 + 41) % W,
  y: (i * 113 + 83) % H,
  r: 0.6 + ((i * 7) % 17) / 18,
  a: 0.05 + ((i * 11) % 15) / 120,
...
function spawnEnemy(crystal = false) {
  ...
  const pad = 70;
  const pos = [
    { x: rand(-pad, W + pad), y: -pad },
    { x: W + pad, y: rand(-pad, H + pad) },
    { x: rand(-pad, W + pad), y: H + pad },
    { x: -pad, y: rand(-pad, H + pad) },
  ][side];
...
function seedMinuteSixState() {
  ...
  for (let i = 0; i < 48; i++) {
    ...
    const x = clamp(game.player.x + Math.cos(a) * ring, 40, W - 40);
  ...
  for (let i = 0; i < 34; i++) {
    ...
    dropXp(
      clamp(game.player.x + Math.cos(a) * r, 80, W - 80),
...
function update(dt) {
  ...
  game.player.x = clamp(game.player.x + game.player.vx * dt, 42, W - 42);
  ...
  for (const shard of game.xp) {
    ...
    if (d < shard.radius + game.player.radius + 2) {
      ...
      if (game.clarity >= 1) {
        ...
        game.flash = 0.26;
        game.damageTexts.push({ x: CENTER.x, y: H * 0.32, r: 20, life: 1.25, color: "#78ff5d" });
      }
    }
  }
...
function sugarCrash() {
  ...
  game.sugarTimer = 0;
  game.focus = 0.72;
  game.player.x = CENTER.x;
  game.player.y = CENTER.y;
  game.player.invuln = 1.4;
  game.enemies.length = Math.min(game.enemies.length, 22);
  game.sugars.length = 0;
  game.flash = 1;
  game.shake = 20;
  burst(CENTER.x, CENTER.y, "#ffffff", 90, 360);
}
...
function drawBackground() {
  ...
  ctx.strokeStyle = "#665d78";
  ctx.lineWidth = 2;
  for (let y = 105; y < H; y += 92) {
    ctx.beginPath();
    ctx.moveTo(0, y);
    ctx.lineTo(W, y - 42);
    ctx.stroke();
  ...
  const rg = ctx.createRadialGradient(CENTER.x, CENTER.y, 20, CENTER.x, CENTER.y, 420);
...
function drawEnemies() {
  for (const enemy of game.enemies) {
    ...
    const sheet =
      enemy.type.key === "news" ? images.newsSheet : enemy.type.key === "cart" ? images.cartSheet : null;
    const animFrame = Math.floor(game.time * (enemy.type.key === "cart" ? 12 : 10) + enemy.id) % 16;
...
function drawPlayer() {
  ...
  drawSheetFrame(images.eyeSheet, Math.floor(game.time * 11) % 16, 0, 0, size * 1.22, 0);
  if (p.invuln > 0) {
    ctx.globalCompositeOperation = "lighter";
